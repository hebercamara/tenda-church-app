...
import { Check, Trash2, Plus, Save, ChevronLeft, ChevronRight, X } from 'lucide-react';
import { formatDateToBrazilian, formatDateToAbbreviated } from '../utils/dateUtils';

const ManageCourseModal = ({ course, members, isOpen, onClose, onSaveStudents, onSaveAttendance, onSkipClassDay, onViewMember }) => {
                                    ...
                                    {draftEnrolledStudents.sort((a, b) => {
                                        ...
                                        <div key={student.id} className="flex justify-between items-center bg-red-50 p-3 rounded-md space-x-2">
                                            <button
                                                ...
                                                title="Ver cadastro"
                                                onClick={() => onViewMember && onViewMember(members.find(m => m.id === student.id))}
                                            >
