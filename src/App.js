...
const CourseForm = ({ onClose, onSave, members, editingCourse }) => {
    const [formData, setFormData] = useState({ name: editingCourse?.name || '', teacherId: editingCourse?.teacherId || '', startDate: editingCourse?.startDate || '', endDate: editingCourse?.endDate || '', classDay: editingCourse?.classDay || '', classTime: editingCourse?.classTime || '' });
    const [error, setError] = useState('');

    const handleChange = (e) => { const { name, value } = e.target; setFormData(prev => ({ ...prev, [name]: value })); };
    const handleSubmit = (e) => {
        e.preventDefault();
        if (!formData.name || !formData.teacherId || !formData.startDate || !formData.endDate || !formData.classDay || !formData.classTime) { setError('Todos os campos são obrigatórios.'); return; }
        const teacher = members.find(m => m.id === formData.teacherId);
        if (!teacher?.email) { setError('O membro selecionado como professor precisa ter um e-mail cadastrado.'); return; }
        onSave({ ...formData, teacherName: teacher.name, teacherEmail: teacher.email, students: editingCourse?.students || [] });
        onClose();
    };
    const weekDays = Object.keys(weekDaysMap);
...
const ManageCourseModal = ({ course, members, isOpen, onClose, onSaveStudents, onSaveAttendance }) => {
    ...
    useEffect(() => {
        if (course && isOpen) {
            ...
            const initialGrades = {};
            (course.students || []).forEach(student => { initialGrades[student.id] = student.grade || ''; });
            setGrades(initialGrades);
            
            // Redefinir a data selecionada ao abrir/mudar o curso para evitar estado obsoleto
            ...
            const unsubscribe = onSnapshot(q, (snapshot) => {
                ...
                // Correção: Usar a forma funcional de setState para evitar o problema de 'stale closure'.
                // Isto garante que a data selecionada não seja redefinida sempre que a presença é marcada.
                setSelectedDate(currentSelectedDate => {
                    // Se uma data já estiver selecionada, mantenha-a.
                    if (currentSelectedDate && sortedRecords.some(r => r.id === currentSelectedDate)) {
                        return currentSelectedDate;
                    }
                    // Se não houver data selecionada E houver registos, selecione o primeiro.
                    if (!currentSelectedDate && sortedRecords.length > 0) {
                        return sortedRecords[0].id;
                    }
                    // Caso contrário, mantenha o estado atual (pode ser nulo se não houver registos).
                    return currentSelectedDate;
                });
            });
            return () => unsubscribe();
        }
    }, [course, isOpen]); // A dependência está correta, não precisa incluir 'selectedDate' aqui.

    const handleEnroll = (studentId) => { const student = members.find(m => m.id === studentId); if (student && !enrolledStudents.some(s => s.id === studentId)) { const newStudents = [...enrolledStudents, { id: student.id, name: student.name, grade: '' }]; setEnrolledStudents(newStudents); onSaveStudents(newStudents); } };
    ...
    const handleGradeChange = (studentId, grade) => { const newGrades = { ...grades, [studentId]: grade }; setGrades(newGrades); const studentIndex = enrolledStudents.findIndex(s => s.id === studentId); if (studentIndex !== -1) { const newStudents = [...enrolledStudents]; newStudents[studentIndex] = { ...newStudents[studentIndex], grade }; onSaveStudents(newStudents); } };
    ...
    const notEnrolledMembers = members.filter(m => !enrolledStudents.some(s => s.id === m.id));
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} size="4xl">
            ...
            <div className="border-b border-gray-200"><nav className="-mb-px flex space-x-8" aria-label="Tabs"><button onClick={() => setActiveTab('students')} className={`${activeTab === 'students' ? 'border-red-500 text-red-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm`}>Alunos e Notas</button><button onClick={() => setActiveTab('attendance')} className={`${activeTab === 'attendance' ? 'border-red-500 text-red-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm`}>Lista de Presença</button></nav></div>
            
            {activeTab === 'students' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-6">
                <div><h3 className="text-lg font-semibold text-gray-800 mb-3">Inscrever Alunos</h3><div className="space-y-2 max-h-96 overflow-y-auto pr-2">{notEnrolledMembers.map(member => (<div key={member.id} className="flex justify-between items-center bg-gray-50 p-2 rounded-md"><span>{member.name}</span><button onClick={() => handleEnroll(member.id)} className="text-sm bg-green-500 hover:bg-green-600 text-white py-1 px-2 rounded-md transition-colors"><Plus size={16}/></button></div>))}</div></div>
                <div><h3 className="text-lg font-semibold text-gray-800 mb-3">Alunos Inscritos e Notas</h3><div className="space-y-2 max-h-96 overflow-y-auto pr-2">{enrolledStudents.map(student => (<div key={student.id} className="flex justify-between items-center bg-blue-50 p-2 rounded-md space-x-2"><span className="flex-1">{student.name}</span><input type="text" value={grades[student.id] || ''} onChange={(e) => handleGradeChange(student.id, e.target.value)} placeholder="Nota" className="w-24 bg-white text-gray-900 rounded-md p-1 border border-gray-300 focus:ring-2 focus:ring-[#DC2626]"/><button onClick={() => handleUnenroll(student.id)} className="text-sm bg-red-500 hover:bg-red-600 text-white p-1.5 rounded-md transition-colors"><Trash2 size={16}/></button></div>))}</div></div>
            </div>)}

            {activeTab === 'attendance' && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-6">
                <div><h3 className="text-lg font-semibold text-gray-800 mb-3">Datas das Aulas</h3><div className="space-y-1 max-h-96 overflow-y-auto pr-2">{attendanceRecords.map(record => (<button key={record.id} onClick={() => setSelectedDate(record.id)} className={`w-full text-left p-2 rounded-md ${selectedDate === record.id ? 'bg-red-500 text-white' : 'bg-gray-100 hover:bg-gray-200'}`}>{new Date(record.date.seconds * 1000).toLocaleDateString('pt-BR', {timeZone: 'UTC'})}</button>))}</div></div>
...
export default function App() {
    ...
    const generateAttendanceRecords = async (courseId, courseData) => {
        ...
        while (currentDate <= endDate) {
            if (currentDate.getUTCDay() === targetDay) {
                ...
                batch.set(attendanceRef, { date: currentDate, statuses: initialStatuses });
            }
    ...
    const handleSaveCourse = async (courseData) => {
        ...
        try {
            if (editingCourse) { await setDoc(doc(db, collectionPath, editingCourse.id), courseData); }
            else { 
    ...
    const handleSaveCourseStudents = async (courseId, students) => {
        ...
        try { await updateDoc(courseRef, { students: students }); } catch (error) { console.error("Erro ao salvar alunos/notas do curso:", error); }
    };

    const handleSaveAttendance = async (courseId, dateId, statuses) => {
        const attendanceRef = doc(db, `artifacts/${appId}/public/data/courses/${courseId}/attendance`, dateId);
        try { await updateDoc(attendanceRef, { statuses: statuses }); } catch (error) { console.error("Erro ao salvar presença:", error); }
    };
    ...
    const displayedConnects = isAdmin ? allConnects : leaderConnects;
    const leaderConnectIds = leaderConnects.map(c => c.id);
    const displayedMembers = isAdmin ? allMembers : allMembers.filter(m => leaderConnectIds.includes(m.connectId));
    const displayedCourses = isAdmin ? allCourses : taughtCourses;
        
    const filteredMembers = displayedMembers.filter(member => member.name.toLowerCase().includes(searchTerm.toLowerCase()));
