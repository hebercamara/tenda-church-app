...
import { User, X, Users, Home, Calendar, Edit, Trash2, Plus, LogOut, MapPin, Clock, Mail } from 'lucide-react';
...
export default function App() {
    ...
    const [leaderConnects, setLeaderConnects] = useState([]);
    ...
    useEffect(() => {
        ...
        setLoadingData(true);
        const unsubConnects = onSnapshot(collection(db, `artifacts/${appId}/public/data/connects`), (snapshot) => {
            const connectsData = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
            setAllConnects(connectsData);
        });
        const unsubMembers = onSnapshot(collection(db, `artifacts/${appId}/public/data/members`), (snapshot) => {
            const membersData = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
            setAllMembers(membersData);
            setLoadingData(false);
        });

        return () => { unsubConnects(); unsubMembers(); };
    }, [user]);
    ...
    const MemberForm = ({ onClose, onSave, connects, editingMember, isAdmin, leaderConnects }) => {
        ...
        const handleSubmit = (e) => {
            e.preventDefault();
            if (!formData.name || !formData.dob || !formData.phone || !formData.gender || !formData.email) {
                setError('Todos os campos, exceto Connect, são obrigatórios.');
                return;
            }
            if (!isAdmin && !formData.connectId) {
                setError('Líderes devem obrigatoriamente designar um Connect para o membro.');
                return;
            }
            onSave(formData);
            onClose();
        ...
        return (
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {error && <p className="text-red-600 bg-red-100 p-2 rounded-md">{error}</p>}
                <div><label htmlFor="name" className="block text-sm font-medium text-gray-700 mb-1">Nome Completo</label><input type="text" name="name" id="name" value={formData.name} onChange={handleChange} className="w-full bg-gray-100 text-gray-900 rounded-md p-2 border border-gray-300 focus:ring-2 focus:ring-[#DC2626]" placeholder="Nome completo" /></div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div><label htmlFor="dob" className="block text-sm font-medium text-gray-700 mb-1">Data de Nascimento</label><input type="date" name="dob" id="dob" value={formData.dob} onChange={handleChange} className="w-full bg-gray-100 text-gray-900 rounded-md p-2 border border-gray-300 focus:ring-2 focus:ring-[#DC2626]" /></div>
                    <div><label htmlFor="email" className="block text-sm font-medium text-gray-700 mb-1">E-mail</label><input type="email" name="email" id="email" value={formData.email} onChange={handleChange} className="w-full bg-gray-100 text-gray-900 rounded-md p-2 border border-gray-300 focus:ring-2 focus:ring-[#DC2626]" placeholder="email@exemplo.com" /></div>
                </div>
                ...
                <div><label htmlFor="connectId" className="block text-sm font-medium text-gray-700 mb-1">Connect</label><select name="connectId" id="connectId" value={formData.connectId} onChange={handleChange} className="w-full bg-gray-100 text-gray-900 rounded-md p-2 border border-gray-300 focus:ring-2 focus:ring-[#DC2626]"><option value="">{isAdmin ? "Nenhum" : "Selecione um Connect"}</option>{availableConnects.map(connect => (<option key={connect.id} value={connect.id}>{connect.number} - {connect.name}</option>))}</select></div>
    ...
    const handleSaveConnect = async (connectData) => { 
        ...
        try { 
            ...
            // Auto-assign leader to the connect
    ...
    const handleConfirmDelete = async () => { if (!deleteAction) return; const { type, id } = deleteAction; try { if (type === 'member') { await deleteDoc(doc(db, `artifacts/${appId}/public/data/members`, id)); } else if (type === 'connect') { const membersToUpdate = allMembers.filter(m => m.connectId === id); for (const member of membersToUpdate) { const memberRef = doc(db, `artifacts/${appId}/public/data/members`, member.id); const updatedData = { ...member, connectId: '' }; delete updatedData.id; await setDoc(memberRef, updatedData); } await deleteDoc(doc(db, `artifacts/${appId}/public/data/connects`, id)); } } catch (error) { console.error(`Erro ao deletar ${type}:`, error); } finally { setConfirmModalOpen(false); setDeleteAction(null); } };
    ...
    const displayedMembers = isAdmin 
        ? allMembers 
        : allMembers.filter(m => leaderConnectIds.includes(m.connectId));
            ...
            <main className="p-4 md:p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    <div className="bg-white p-6 rounded-xl shadow-md flex flex-col justify-between border border-gray-200">
                        <div><div className="flex items-center space-x-3 mb-2"><Users size={28} className="text-[#DC2626]" /><h2 className="text-2xl font-bold text-gray-900">Membros</h2></div><p className="text-gray-600">Gerencie todos os membros da igreja.</p><p className="text-4xl font-black text-gray-800 mt-4">{displayedMembers.length}</p></div>
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-md flex flex-col justify-between border border-gray-200">
                        <div><div className="flex items-center space-x-3 mb-2"><Home size={28} className="text-[#DC2626]" /><h2 className="text-2xl font-bold text-gray-900">Connects</h2></div><p className="text-gray-600">Gerencie as células (Connects).</p><p className="text-4xl font-black text-gray-800 mt-4">{displayedConnects.length}</p></div>
                        {isAdmin && <button onClick={() => openConnectModal()} className="mt-4 w-full bg-gray-600 hover:bg-gray-700 text-white font-bold py-3 px-4 rounded-lg flex items-center justify-center space-x-2 transition-all"><Plus size={20} /><span>Adicionar Connect</span></button>}
                    </div>
                </div>
                ...
                <div className="mb-8">
                    ...
                    {loadingData ? <LoadingSpinner /> : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {displayedConnects.sort((a, b) => a.number - b.number).map(c => (
                                <div key={c.id} className="bg-white rounded-lg p-4 flex flex-col justify-between transition-all shadow-md hover:shadow-lg hover:-translate-y-1 border border-gray-200">
                                    <div>
                                        ...
                                        <p className="text-gray-600 flex items-center"><Mail size={14} className="inline mr-2 flex-shrink-0"/>{c.leaderEmail}</p>
